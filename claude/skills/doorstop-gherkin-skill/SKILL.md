...
Spec-Weaverは `@SPEC-001` タグをGherkinとDoorstopの橋渡しに使う。

### インストール

```bash
git clone <spec-weaver-repo>
cd spec-weaver
uv tool install .
```
...
# Doorstopルートが別ディレクトリにある場合
spec-weaver audit ./backend/tests/features --repo-root ./docs/doorstop

# プレフィックスを変更する場合（デフォルト: SPEC）
spec-weaver audit ./specification/features --prefix REQ
```
...
### テスト対象外の仕様（`testable: false`）

Gherkinでのテストが不可能な仕様（UIカラー、ライセンス文言など）はYAMLに追記:

```yaml
# SPEC-005.yml
active: true
testable: false   # ← Spec-Weaverの監査対象から除外される
text: |
